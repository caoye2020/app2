...
<div class="row">
	...
	<div class="col-md-12">
		<div class="x_panel">
			<div class="x_title">
				<h2>
					APP 审核列表 <i class="fa fa-user"></i><small>${backednUser.userName}
						- 您可以通过搜索或者其他的筛选项对APP的信息进行审核操作。^_^</small>
				</h2>
				<div class="clearfix"></div>
			</div>
			<div class="x_content">
				<form method="post" action="${pageContext.request.contextPath}/sys/backend/list">
					...
					<li>
						<div class="form-group">
							...
							<div class="col-md-6 col-sm-6 col-xs-12">
								<input name="appName" type="text" class="form-control col-md-7 col-xs-12" value="${querySoftwareName }">
							</div>
						</div>
					</li>
							...
							<div class="col-md-6 col-sm-6 col-xs-12">
								<select id="queryCategoryLevel1" name="yiji" class="form-control">
									<c:if test="${listD!= null }">
									   <option value="-1">--请选择--</option>
									   <c:forEach var="appCategory" items="${listD}">
						...
						<div class="form-group">
							...
							<input type="hidden" name="categorylevel2list" id="categorylevel2list"/>
        						<select name="erji" id="queryCategoryLevel2" class="form-control">
        							<c:if test="${listD2 != null }">
									   <option value="-1">--请选择--</option>
									   <c:forEach var="appCategory" items="${listD2}">
						...
						<div class="form-group">
							...
							<div class="col-md-6 col-sm-6 col-xs-12">
        						<select name="sanji" id="queryCategoryLevel3" class="form-control">
        							<c:if test="${listD3 != null }">
									   <option value="-1">--请选择--</option>
									   <c:forEach var="appCategory" items="${listD3}">
							...
							<ul class="pagination">
								<c:if test="${page.currPageNo > 1}">
									<li class="paginate_button previous"><a
										href="${pageContext.request.contextPath}/sys/backend/list?currPageNo=1&appName=${appName}&status=${status}&flatform=${flatform}&yiji=${yiji}&erji=${erji}&sanji=${sanji}"
										aria-controls="datatable-responsive" data-dt-idx="0"
										tabindex="0">首页</a>
									</li>
									<li class="paginate_button "><a
										href="${pageContext.request.contextPath}/sys/backend/list?currPageNo=${page.currPageNo-1}&appName=${appName}&status=${status}&flatform=${flatform}&yiji=${yiji}&erji=${erji}&sanji=${sanji}"
								...
								<c:if test="${page.currPageNo < page.totalPageCount }">
									...
									<li class="paginate_button next"><a
										href="${pageContext.request.contextPath}/sys/backend/list?currPageNo=${page.totalPageCount}&appName=${appName}&status=${status}&flatform=${flatform}&yiji=${yiji}&erji=${erji}&sanji=${sanji}"
