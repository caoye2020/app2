...
$(".modifyVersion").on("click",function(){
	...
	if(status == "1" || status == "3"){//待审核、审核未通过状态下才可以进行修改操作
		if(versionid == null || versionid == ""){
			...
			window.location.href=ctx+"/sys/devindex/addversionmodify?id="+appId+"";
		}
	}else{
...
$(".modifyAppInfo").on("click",function(){
	...
	var status = obj.attr("status");
	if(status == "1" || status == "3"){//待审核、审核未通过状态下才可以进行修改操作
		window.location.href=ctx+"/sys/devindex/appinfomodify?id="+ obj.attr("appinfoid");
	}else{
...
$(document).on("click",".saleSwichOpen,.saleSwichClose",function(){
	...
	if("open" === saleSwitch){
		saleSwitchAjax(appinfoid,4,obj);
	}else if("close" === saleSwitch){
		if(confirm("你确定要下架您的APP应用【"+obj.attr("appsoftwarename")+"】吗？")){
			saleSwitchAjax(appinfoid,5,obj);
		}
	}
});
var saleSwitchAjax = function(appId,count,obj){
	$.ajax({
		type:"GET",
		url:ctx+"/sys/devindex/updateStatus.json",
		data:{id:appId,status:count},
		dataType:"json",
...
$(".viewApp").on("click",function(){
	var obj = $(this);
	var version=obj.attr("versionNo");
	window.location.href=ctx+"/sys/devindex/view/"+ obj.attr("appinfoid");
});
$(".deleteApp").on("click",function(){
	...
	if(confirm("你确定要删除APP应用【"+obj.attr("appsoftwarename")+"】及其所有的版本吗？")){
		$.ajax({
			type:"GET",
			url:ctx+"/sys/devindex/deleteApp.json",
			data:{id:obj.attr("appinfoid")},
			dataType:"json",
			success:function(data){
				if(data.result>0){//删除成功：移除删除行
					alert("删除成功");
